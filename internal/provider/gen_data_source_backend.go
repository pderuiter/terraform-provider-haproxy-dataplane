...
var _ datasource.DataSource = (*backendDataSource)(nil)
...
func (d *backendDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	attrs, ok := schemaAttributesForDataSource("backend")
	if !ok {
...
func (d *backendDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	items, diags := listObjectsFrom(ctx, mustSchemaAttrTypes("backend"), out)
	resp.Diagnostics.Append(diags...)
	state.Items = items
	state.ID = types.StringValue(strings.Join([]string{"backend"}, "/"))
