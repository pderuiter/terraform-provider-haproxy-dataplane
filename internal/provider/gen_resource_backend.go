...
func (r *backendResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	attrs, ok := schemaAttributesFor("backend")
	if !ok {
		resp.Diagnostics.AddError("Schema not found", "backend")
		return
	}
...
func (r *backendResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	path := buildPath("/services/haproxy/configuration/backends", map[string]string{
	...
	plan.Spec, diags = mapToObject(ctx, mustSchemaAttrTypes("backend"), payload, []string{"name"})
	resp.Diagnostics.Append(diags...)
...
func (r *backendResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	query := url.Values{}
	path := buildPath("/services/haproxy/configuration/backends/{name}", map[string]string{
	...
	state.Spec, diags = mapToObject(ctx, mustSchemaAttrTypes("backend"), out, []string{"name"})
	resp.Diagnostics.Append(diags...)
