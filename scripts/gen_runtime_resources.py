...
for path, item in runtime_paths.items():
    ...
    if not collection_path.startswith('/services/haproxy/runtime/'):
        continue

    collection_item = runtime_paths.get(collection_path)
    has_post = bool(collection_item and 'post' in collection_item)
    has_put = 'put' in item
    has_delete = 'delete' in item

    def schema_name_from_op(op):
        if not op:
            return None
    ...
    post_schema = schema_name_from_op(collection_item['post']) if has_post else None
    put_schema = schema_name_from_op(item.get('put', {})) if has_put else None
    ...
    res_name = 'runtime_' + '_'.join(rel_literals)
...
with open(OUT_DIR / 'runtime_resources_gen.go', 'w') as f:
    f.write("// Code generated by scripts/gen_runtime_resources.py. DO NOT EDIT.\n")
    f.write("package provider\n\n")
    f.write("import (\n\t\"github.com/hashicorp/terraform-plugin-framework/resource\"\n)\n\n")
    f.write("func generatedRuntimeResources() []func() resource.Resource {\n\treturn []func() resource.Resource{\n")
    for r in resources:
...
for r in resources:
    ...
    with open(OUT_DIR / f'gen_resource_{name}.go', 'w') as f:
        ...
        if r['create_via'] == 'post':
            f.write("\tif err := r.client.PostJSON(ctx, path, query, payload, nil); err != nil {\n\t\tresp.Diagnostics.AddError(\"Create failed\", err.Error())\n\t\treturn\n\t}\n")
        else:
            f.write("\tif err := r.client.PutJSON(ctx, path, query, payload, nil); err != nil {\n\t\tresp.Diagnostics.AddError(\"Create failed\", err.Error())\n\t\treturn\n\t}\n")
        f.write(f"\tplan.ID = types.StringValue({id_expr})\n")
        f.write("\tplan.Spec, diags = mapToObject(ctx, mustSchemaAttrTypes(\"%s\"), payload, %s)\n" % (r['schema'], drop_keys_str))
        ...
        f.write("\tvar diags diag.Diagnostics\n\tstate.Spec, diags = mapToObject(ctx, mustSchemaAttrTypes(\"%s\"), out, %s)\n" % (r['schema'], drop_keys_str))
        ...
        if r['has_delete']:
            ...
            for line in path_lines('state.'):
                f.write(line)
            f.write("\t})\n")
            f.write("\tif err := r.client.DeleteJSON(ctx, path, query, nil); err != nil {\n\t\tif client.IsNotFound(err) {\n\t\t\treturn\n\t\t}\n\t\tresp.Diagnostics.AddError(\"Delete failed\", err.Error())\n\t\treturn\n\t}\n")
        f.write("}\n\n")

        f.write(f"var _ resource.ResourceWithConfigure = (*{name}Resource)(nil)\n")

print(f"Generated {len(resources)} runtime resources")
