locals {
  frontend_bind_name = "managed_frontend_bind"
}

variable "parent_name" {
  description = "Parent object name, for example a frontend or backend name."
  type        = string
}

resource "haproxy-dataplane_frontend_bind" "managed" {
  name        = local.frontend_bind_name
  parent_name = var.parent_name

  # Replace with required fields for this object in your environment.
  spec = {}
}

output "frontend_bind_id" {
  value = haproxy-dataplane_frontend_bind.managed.id
}
