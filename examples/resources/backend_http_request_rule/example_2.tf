locals {
  backend_http_request_rule_name = "managed_backend_http_request_rule"
}
...
resource "haproxy-dataplane_backend_http_request_rule" "managed" {
...
output "backend_http_request_rule_id" {
  value = haproxy-dataplane_backend_http_request_rule.managed.id
}
