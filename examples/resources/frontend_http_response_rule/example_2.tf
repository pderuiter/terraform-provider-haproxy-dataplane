locals {
  frontend_http_response_rule_name = "managed_frontend_http_response_rule"
}
...
resource "haproxy-dataplane_frontend_http_response_rule" "managed" {
...
output "frontend_http_response_rule_id" {
  value = haproxy-dataplane_frontend_http_response_rule.managed.id
}
